...
TC-NEG-TF-01 Transfer To Same Account
    ...
    ${account}=    Get Text    ${from_account_dropdown}

    Select From List By Label    ${from_account}    ${account}

    Select From List By Label    ${to_account}    ${account}
...
TC-NEG-TF-02 Transfer Amount Exceeds Balance
    ...
    Wait Until Keyword Succeeds    10x    1s    Page Should Contain Element    ${from_account}
    ...
    Select From List By Label    ${from_account}    ${account}
    Select From List By Index    ${to_account}    ${account}[1]
    Click Element    ${transfer_btn}